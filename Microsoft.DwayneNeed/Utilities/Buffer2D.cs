    ...
    public struct Buffer2D<T> where T : struct
    {
        ...
        public Buffer2D(Buffer2D<T> buffer, Int32Rect bounds)
        {
                ...
                (bounds.Y + bounds.Height) > (buffer._bounds.Y + buffer._bounds.Height))
            {
                throw new ArgumentOutOfRangeException("bounds",
                    "The specified bounds must be within the bounds of the buffer.");
            }
        ...
        public void SetBits(Buffer2DView<T> srcView, Int32Rect srcRect, Int32Point? dstPoint = null)
        {
                ...
                srcRect.Y < 0 || srcRect.Height < 0 || (srcRect.Y + srcRect.Height) > srcView._buffer._bounds.Height)
            {
                throw new ArgumentOutOfRangeException("srcRect",
                    "The source rect must specify coordinates within the bounds of the source buffer.");
            }
            ...
            {
                throw new ArgumentOutOfRangeException("dstPoint",
                    "The destination point must specify coordinates within the bounds of the buffer.");
            }

            unsafe
            {
                int sizeOfT = Marshal.SizeOf(typeof(T));
                byte* pRead = ((byte*) srcView._buffer._pBits.ToPointer()) +
                              (srcRect.Y + srcView._buffer._bounds.Y) * srcView._buffer._rowWidth * sizeOfT +
                              (srcRect.X + srcView._buffer._bounds.X) * sizeOfT;
                byte* pWrite = ((byte*) _pBits.ToPointer()) + (theDstPoint.Y + _bounds.Y) * _rowWidth * sizeOfT +
                               (theDstPoint.X + _bounds.X) * sizeOfT;
        ...
        public T this[int x, int y]
        {
            get
            {
                if (x < 0 || x > _bounds.Width)
                {
                    throw new ArgumentOutOfRangeException("x",
                        "The indexer must specify coordinates within the bounds of the buffer.");
                }
                ...
                {
                    int sizeOfT = Marshal.SizeOf(typeof(T));
                    byte* pRead = ((byte*) _pBits.ToPointer()) + (y + _bounds.Y) * _rowWidth * sizeOfT +
                                  (x + _bounds.X) * sizeOfT;
            ...
            set
            {
                ...
                {
                    int sizeOfT = Marshal.SizeOf(typeof(T));
                    byte* pWrite = ((byte*) _pBits.ToPointer()) + (y + _bounds.Y) * _rowWidth * sizeOfT +
                                   (x + _bounds.X) * sizeOfT;
            ...
            BitmapPalette bitmapPalette)
        {
            ...
            return BitmapSource.Create(_bounds.Width, _bounds.Height, dpiX, dpiY, pixelFormat, bitmapPalette, buffer,
                bufferSize, stride);
        }
