...
namespace PresetMagicianScratchPad.Roland
{
    ...
    {
        public static List<string> ComparePatchDumps(byte[] master, byte[] compareData, RolandScript rolandScript)
        {
            ...
            foreach (var addr in masterAddresses)
            {
                if (!compareMemoryMap.ContainsKey(addr))
                {
                    ...
                    errors.Add($"Compare subject does not contain address 0x{addr:x8} ({result.closestSection.ValuePath})");
                    continue;
                }
                ...
                {
                    var result = rolandScript.FindClosestToAddress(addr);
                    errors.Add($"Compare subject value at address 0x{addr:x8} ({result.closestSection.ValuePath}) with value 0x{compareMemoryMap[addr]:x8} does not equal master value {masterMemoryMap[addr]:x8}");
                    continue;
                }
            }
            ...
            {
                var result = rolandScript.FindClosestToAddress(addr);
                errors.Add($"Compare subject contains address 0x{addr:x8} ({result.closestSection.ValuePath}) but it shouldn't");
            }
